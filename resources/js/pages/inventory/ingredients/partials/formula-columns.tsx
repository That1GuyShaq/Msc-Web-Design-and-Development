...
export const formulaColumns: ColumnDef<Formula>[] = [
	...
	},
	{
        ...
        cell: ({row}) => {
			...
			const [color, text] = status === 'draft' ? ['text-amber-500', 'Draft'] : status === 'published' ? ['text-green-500', 'Published'] : ['text-red-500', 'Archived'];
			return (
				<p className="flex items-center gap-2 text-ellipsis text-muted-foreground text-wrap">
					<Badge variant="default"><CircleSmall className={`h-4 w-4 ${color} fill-current`} />{text}</Badge>
				</p>
			)
		}
	},
